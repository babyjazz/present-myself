 ...
 * @since 1.0.0
 */
class Animation extends _events2.default {
  /**
   ...
   */
  setDuration() {
    let duration = arguments.length <= 0 || arguments[0] === undefined ? 1000 : arguments[0];
   ...
   */
  setEasing() {
    let easing = arguments.length <= 0 || arguments[0] === undefined ? 'outQuad' : arguments[0];

    if (typeof _easing.EASING[easing] !== 'function') throw new Error(`Unknown easing: ${ easing }`);
  ...
  onEasing(easing, time, startValue, byValue, duration) {
    return Math.round(_easing.EASING[easing](time, startValue, byValue, duration));
  }
